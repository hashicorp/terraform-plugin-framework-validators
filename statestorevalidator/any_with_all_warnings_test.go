...
func TestAnyWithAllWarningsValidatorValidateStateStore(t *testing.T) {
	t.Parallel()
	...
	testCases := map[string]struct {
		...
		"valid": {
			validators: []statestore.ConfigValidator{
				statestorevalidator.ExactlyOneOf(
		...
		"valid with warning": {
			validators: []statestore.ConfigValidator{
				statestorevalidator.All(
					...
					testvalidator.WarningStateStore("failing warning summary", "failing warning details"),
				),
				statestorevalidator.All(
					statestorevalidator.ExactlyOneOf(
						path.MatchRoot("test3"),
					...
					testvalidator.WarningStateStore("passing warning summary", "passing warning details"),
				),
			},
			req: statestore.ValidateConfigRequest{
				Config: tfsdk.Config{
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			statestorevalidator.AnyWithAllWarnings(testCase.validators...).ValidateStateStore(context.Background(), testCase.req, got)
