...
func TestAnyWithAllWarningsValidatorValidateListResource(t *testing.T) {
	t.Parallel()
		...
		"valid with warning": {
			validators: []list.ConfigValidator{
				listresourcevalidator.All(
					...
					testvalidator.WarningListResourceConfig("failing warning summary", "failing warning details"),
				),
				listresourcevalidator.All(
					listresourcevalidator.ExactlyOneOf(
						path.MatchRoot("test3"),
					...
					testvalidator.WarningListResourceConfig("passing warning summary", "passing warning details"),
				),
			},
			req: list.ValidateConfigRequest{
				Config: tfsdk.Config{
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			listresourcevalidator.AnyWithAllWarnings(testCase.validators...).ValidateListResourceConfig(context.Background(), testCase.req, got)
