...
func TestAtLeastSumOfValidator(t *testing.T) {
	...
	tests := map[string]testCase{
		...
		"valid integer as Int32 greater than sum of attributes": {
			val: types.Int32Value(10),
			attributesToSumExpressions: path.Expressions{
				path.MatchRoot("one"),
				path.MatchRoot("two"),
		...
		"valid integer as Int32 greater than sum of attributes, when one summed attribute is null": {
			val: types.Int32Value(10),
			attributesToSumExpressions: path.Expressions{
				path.MatchRoot("one"),
				path.MatchRoot("two"),
		...
		"valid integer as Int32 greater than sum of attributes, when one summed attribute is unknown": {
			val: types.Int32Value(10),
			attributesToSumExpressions: path.Expressions{
				path.MatchRoot("one"),
				path.MatchRoot("two"),
	...
	for name, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			AtLeastSumOf(test.attributesToSumExpressions...).ValidateInt32(context.Background(), request, &response)
