...
func TestEqualToSumOfValidator(t *testing.T) {
	...
	tests := map[string]testCase{
		...
		"valid integer as Int32 equal to sum of attributes, when one summed attribute is null": {
		...
		"valid integer as Int32 equal to sum of attributes, when one summed attribute is unknown": {
			val: types.Int32Value(8),
			attributesToSumPathExpressions: path.Expressions{
				path.MatchRoot("one"),
				path.MatchRoot("two"),
	...
	for name, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			EqualToSumOf(test.attributesToSumPathExpressions...).ValidateInt32(context.Background(), request, &response)
