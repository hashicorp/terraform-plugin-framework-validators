...
func TestAnyWithAllWarningsValidatorValidateDataSource(t *testing.T) {
	t.Parallel()
		...
		"valid with warning": {
			validators: []datasource.ConfigValidator{
				datasourcevalidator.All(
					...
					testvalidator.WarningDataSource("failing warning summary", "failing warning details"),
				),
				datasourcevalidator.All(
					datasourcevalidator.ExactlyOneOf(
						path.MatchRoot("test3"),
					...
					testvalidator.WarningDataSource("passing warning summary", "passing warning details"),
				),
			},
			req: datasource.ValidateConfigRequest{
				Config: tfsdk.Config{
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			datasourcevalidator.AnyWithAllWarnings(testCase.validators...).ValidateDataSource(context.Background(), testCase.req, got)
