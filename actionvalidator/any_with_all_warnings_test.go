...
func TestAnyWithAllWarningsValidatorValidateAction(t *testing.T) {
	t.Parallel()
		...
		"valid with warning": {
			validators: []action.ConfigValidator{
				actionvalidator.All(
					...
					testvalidator.WarningAction("failing warning summary", "failing warning details"),
				),
				actionvalidator.All(
					actionvalidator.ExactlyOneOf(
						path.MatchRoot("test3"),
					...
					testvalidator.WarningAction("passing warning summary", "passing warning details"),
				),
			},
			req: action.ValidateConfigRequest{
				Config: tfsdk.Config{
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			actionvalidator.AnyWithAllWarnings(testCase.validators...).ValidateAction(context.Background(), testCase.req, got)
